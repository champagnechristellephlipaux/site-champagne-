...
export function estimateSelectionTotal(sku, formatKey, qty = 1) {
  const line = [{ sku, format: formatKey, qty }];
  const subtotal = (PRICE_EUR?.[sku]?.[formatKey] ?? 0) * qty;
  const shipping = shippingTotals(line).shippingTotal;
